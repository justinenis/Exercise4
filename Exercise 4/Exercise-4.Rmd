...
total_correct = sum(DTM_test$labels_test == pred)

# Model Accuracy
percent_accuracy = confusionMatrix(as.factor(DTM_test$labels_test), pred)$overall['Accuracy']
```
...
Using the data to train and test the model, I then used naive bayes to create a predictive model so as to correctly classify the authors. Using this predictive model, I was able to correctly predict 992 authors, an accuracy rate of 39.86%.
